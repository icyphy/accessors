...
keyValueServer = "192.168.1.217:8099"
...
#accessor = re.sub("localhost","192.168.1.5", accessor)
#accessor = re.sub("localhost","192.168.0.101", accessor)
#Replaces all occurances. Must replace input for the RosSubscriber and the RosPublisher

#hack that prevents the rosbridge from unregistering as a publisher for finite
ws = create_connection("ws://localhost:9090")
bridgeAdvertise = json.dumps( {"op": "advertise", "topic": "/finite", "type": "std_msgs/UInt8" })
print bridgeAdvertise
ws.send(bridgeAdvertise)
